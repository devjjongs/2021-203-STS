...
//		dao.connection();
	...
	public  String   findByColumn(String column,String value,Model model ) {
		...
		System.err.println("==>"+column.trim()+"<===");
	...
	public String memberInsert(@ModelAttribute  com.bit.spring.model.orm.vo.Member member , Model model) {
	...
	/**
	 * loginForm.jsp에서 Member 자료를 입력받아서 orm.dao.MemberDao~~.memberInsert()
	 * 결과에 따라서 home.jsp ,  입력 controller의 메소드 호출하는 부분을 직접하지 않고 
	 * 비동기적으로 처리하겠다
	 */
	@RequestMapping(value = "memberInsertAjax" , method=RequestMethod.POST)
	@ResponseBody
	public Object memberInsertAjax( @RequestParam Map<String, Object> paramMap ) {
		...
		if(paramMap.get("userId").toString().length()>=5) {
			...
			retVal.put("nextPage", "memberListOrm");
		...
		logger.info(retVal.get("message").toString());
		return retVal;
	}
}
